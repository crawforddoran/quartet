...
struct Vec
{
    ...
    template<class S>
    explicit Vec<N,T>(S value_for_all)
    ...
    template<class S>
    Vec<N,T>(S v0, S v1, S v2, S v3, S v4)
    {
        assert(N==5);
    ...
    template<class S>
    Vec<N,T>(S v0, S v1, S v2, S v3, S v4, S v5)
    {
        assert(N==6);
    ...
    template<class S>
    Vec<N,T> operator+=(const Vec<N,S> &w)
    {
    ...
    template<class S>
    Vec<N,T> operator+(const Vec<N,S> &w) const
    {
        Vec<N,T> sum(*this);
        sum+=w;
        return sum;
...
template<unsigned int N, class S, class T>
inline Vec<N,T> operator*(S a, const Vec<N,T> &v)
{
    Vec<N,T> w(v);
    w*=a;
    return w;
...
inline Vec<2,T> rotate(const Vec<2,T>& a, float angle) 
{
    T c = cos(angle);
    T s = sin(angle);
    return Vec<2,T>(c*a[0] - s*a[1],s*a[0] + c*a[1]); // counter-clockwise
}

template<class T>
inline Vec<2,T> perp(const Vec<2,T> &a)
{ return Vec<2,T>(-a.v[1], a.v[0]); } // counter-clockwise by 90 degrees
